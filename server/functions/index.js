...
app.get("/", authenticate,  async (req, res) => {
  res.send(
    {result:"API for toodly.co, version 1.0"}
  )
})
...
app.get("/pay", async (req, res) => {
  ...
  const payReq = JSON.stringify({
    ...
    redirect_urls: {
      return_url: `${req.protocol}://${req.get('host')}`+config().base_url+`/process`,
      cancel_url: `${req.protocol}://${req.get('host')}`+config().base_url+`/cancel`
    },
    transactions: [{
      amount: amount,
